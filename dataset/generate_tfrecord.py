...
def create_tf_example(image_fg_path, image_bg_path, trimap_path,
    ...
    alpha_dis = data_provider.alpha_matte_discrete(image_fg)
    ...
    if resize_size is not None:
        ...
        alpha_dis = cv2.resize(alpha_dis, (width, height))
        trimap = cv2.resize(trimap, (width, height))
    else:
    ...
    pil_alpha_dis = PIL.Image.fromarray(alpha_dis)
    bytes_io = io.BytesIO()
    pil_alpha_dis.save(bytes_io, format='JPEG')
    encoded_alpha_dis = bytes_io.getvalue()
    ...
    tf_example = tf.train.Example(
        features=tf.train.Features(feature={
            ...
            'alpha_dis/encoded' : bytes_feature(encoded_alpha_dis),
...
def main(_):
    ...
    image_paths = data_provider.provide(annotation_path)
