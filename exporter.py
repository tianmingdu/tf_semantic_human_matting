...
def _trimap_tensor_input_placeholder(input_shape=None):
    """Returns input placeholder and a 3-D unit8 trimpa tensor."""
    if input_shape is None:
        input_shape = (None, None, None)
    input_tensor = tf.placeholder(
        dtype=tf.uint8, shape=input_shape, name='trimap_tensor')
    return input_tensor, input_tensor
...
input_placeholder_fn_map = {
    ...
    #     'trimap_tensor': _trimap_tensor_input_placeholder,
}
    ...
    with tf.Graph().as_default():
        with session.Session() as sess:
            ...
            for k, v in inputs.items():
                tensor_info_inputs[k] = tf.saved_model.utils.build_tensor_info(
                    v)
            tensor_info_outputs = {}
...
def _get_outputs_from_inputs(input_tensors, model,
    ...
    images_fg = input_tensors.get('images_fg')
    # trimaps = input_tensors.get('trimaps')
    preprocessed_inputs = model.preprocess(images_forground=images_fg)
    output_tensors = model.predict(preprocessed_inputs)
    postprocessed_tensors = model.postprocess(prediction_dict=output_tensors)
...
def _build_model_graph(input_type, model, input_shape,
    ...
    placeholder_tensor, image_tensors = input_placeholder_fn_map[input_type](
        **placeholder_args)
    # placeholder_trimap_tensor, trimap_tensors = (
    #     input_placeholder_fn_map['trimap_tensor']())
    input_tensors = {'images_fg': image_tensors
                     #           ,'trimaps': trimap_tensors
                     }
    outputs = _get_outputs_from_inputs(
        input_tensors=input_tensors,
        model=model,
        output_collection_name=output_collection_name)
    placeholder_tensors = {'images_fg': placeholder_tensor
                           # ,   'trimaps': placeholder_trimap_tensor
                           }
...
def export_inference_graph(input_type,
    ...
    outputs, placeholder_tensors = _build_model_graph(
