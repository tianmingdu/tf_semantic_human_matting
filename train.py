...
flags.DEFINE_float('num_epochs_per_decay', 2.0,
                   ...
                   'across all replicas.')
flags.DEFINE_integer('num_samples', 5000, 'Number of samples.')
...
def main(_):
    ...
    images_fg, images_bg, trimaps, alpha_mattes = tf.train.batch(
        ...
        capacity=FLAGS.batch_size + 1,
        allow_smaller_final_batch=True)
    
    cls_model = model.Model(is_training=True, default_image_size=size)
    preprocessed_dict = cls_model.preprocess(trimaps, None, images_fg,
                                               images_bg, alpha_mattes)
    prediction_dict = cls_model.predict(preprocessed_dict)
    loss_dict = cls_model.loss(prediction_dict, preprocessed_dict)
    loss = loss_dict['loss']
    trimap_losses = loss_dict['trimap_losses']
    alpha_losses = loss_dict['alpha_losses']
    composition_losses = loss_dict['composition_losses']
    ...
    for key, value in postprocessed_dict.items():
        tf.summary.image(key, value, max_outputs=7)
